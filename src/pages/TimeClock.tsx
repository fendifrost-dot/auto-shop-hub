...
const IDLE_OPTIONS: { value: Database["public"]["Enums"]["idle_category"]; label: string }[] = [
...
export default function TimeClock() {
  ...
  const [idleCategory, setIdleCategory] = useState<Database["public"]["Enums"]["idle_category"]>("break");
  ...
  async function handleStartJob() {
    if (!activeShift || !jobId) {
      toast.error("Pick a job first");
      return;
    }
    try {
      await startJobSlice.mutateAsync({ timeEntryId: activeShift.id, jobId });
      toast.success("Job time started");
      setJobId("");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Could not start job time");
    }
  }
  ...
  async function handleStartIdle() {
    if (!activeShift) return;
    try {
      await startIdleSlice.mutateAsync({ timeEntryId: activeShift.id, category: idleCategory });
      toast.success("Idle time logged");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Could not log idle");
    }
  }
        ...
        <Card>
          ...
          <CardContent className="flex flex-col sm:flex-row gap-3">
            {!activeShift ? (
              ...
              >
                {clockOut.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : <Square className="h-5 w-5" />}
                Clock out
              </Button>
            )}
          </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <SelectContent>
                      {jobs.map((j) => (
                        <SelectItem key={j.id} value={j.id}>
                          {j.job_number} — {j.customer_name ?? "Customer"}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  <Button
                    ...
                    disabled={!jobId || startJobSlice.isPending}
                  >
                    {startJobSlice.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Briefcase className="h-4 w-4" />}
                    Start job time
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <Select value={idleCategory} onValueChange={(v) => setIdleCategory(v as typeof idleCategory)}>
                    <SelectTrigger className="w-full">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <Button
                    variant="outline"
                    className="sm:w-auto w-full"
                    onClick={() => void handleStartIdle()}
                    disabled={startIdleSlice.isPending}
                  >
                    {startIdleSlice.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Coffee className="h-4 w-4" />}
                    Start idle
                  </Button>
                </div>
              </div>
