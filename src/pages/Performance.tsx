...
type PerfType = Database["public"]["Enums"]["performance_record_type"];
...
export default function Performance() {
  ...
  const profilesQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("profiles").select("user_id, full_name, email").eq("is_active", true);
      if (error) throw error;
      return data ?? [];
    },
    enabled: canManage,
  ...
  const visible = (recordsQuery.data ?? []).filter((r) => canManage || r.user_id === user?.id);
    ...
    <div className="space-y-6">
            ...
            <DialogContent className="sm:max-w-lg">
              ...
              <div className="grid gap-3 py-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="pdate" type="date" value={recordDate} onChange={(e) => setRecordDate(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="sum">Summary</Label>
                  <Input id="sum" value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="Short headline" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="det">Details</Label>
                  <Textarea id="det" value={details} onChange={(e) => setDetails(e.target.value)} rows={3} />
                </div>
              </div>
              <DialogFooter>
                ...
                <Button
                  onClick={() => void createRecord.mutateAsync()}
                  disabled={!employeeId || !summary.trim() || createRecord.isPending}
                >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{canManage ? "Shop-wide records." : "Your records."}</CardDescription>
        </CardHeader>
        <CardContent>
